...
public class GameManager : MonoBehaviour
{
    ...
    private void ImageSelect() // перебирает дочерние объекты в масиве
    {
        index = PlayerPrefs.GetInt("SelectBackgrounds"); //получение индекса из записи
    ...
    private void SpawnCells() // Создание клеток
    {
    ...
    private void CreateAndStoreLevel(int[,] grid, int level) // Создание и сохранения уровня
    {
    ...
    private void GetCurrentLevel(int[,] grid) // Получение текущего уровня
    {
    ...
    private void GoToNextLevel() // Переход на следующий уровень
    {
        int level = PlayerPrefs.GetInt("Level", 0);
    ...
    private void ResetGrid() // Сброс сетки
    {
        for (int i = 0; i < GRID_SIZE; i++)
        {
            for (int j = 0; j < GRID_SIZE; j++)
    ...
    public void UpdateCellValue(int value) // Обновить значение ячейки
    {
    ...
    public void RestartCurrentLevel() // Перезагрузка текущего уровня
    {
        int level = PlayerPrefs.GetInt("Level", 0);
        CreateAndStoreLevel(new int[GRID_SIZE, GRID_SIZE], level );
        RestartGame();        
    }
    public void StartGameWith1Level() // Начать с первого уровня
    {
        CreateAndStoreLevel(new int[GRID_SIZE, GRID_SIZE], 1 );
        RestartGame();
    } 
