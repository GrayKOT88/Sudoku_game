...
public class Cell : MonoBehaviour
{
    ...
    [Space]
    [Header("Selected")]
    [SerializeField] private Color _selectedCellColor; // цвет выбраной клетки
    [SerializeField] private Color _selectedTextColor; // цвет текста (верного)
    [SerializeField] private Color _selectedWrongCellColor; // цвет клетки (нерпавельный выбор)
    [SerializeField] private Color _selectedWrongTextColor; // цвет текста (неправельный выбор) 
    [Space]
    [Header("Reset")]
    ...
    public void Select()
    {
        if (IsIncorrect) // явл€етс€ неверным
        {
            _bgSrpite.color = _selectedWrongCellColor;            
            _valueText.color = _selectedWrongTextColor; // тут
        }
        else
        {
